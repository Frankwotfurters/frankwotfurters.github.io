...
function addManga() {
    ...
    if (url) {
        ...
        if (!saved_manga.hasOwnProperty(url)) {
            fetch(`http://13.214.130.193:5000/import_manga?url=${url}`)
...
// </div>
    ...
    Object.keys(saved_manga).forEach(url => {
        ...
        fetch(`http://13.214.130.193:5000/get_last?source=${encodeURIComponent(saved_manga[url]['source'])}&id=${encodeURIComponent(saved_manga[url]['manga_id'])}`)
        .then(response => response.json())
        .then(data => {
